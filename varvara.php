...
// if (true && $_SERVER['REMOTE_ADDR'] != '178.68.154.15' && $_SERVER['REMOTE_ADDR'] != '95.52.103.139' && $_SERVER['REMOTE_ADDR'] != '178.68.188.255' && $_SERVER['REMOTE_ADDR'] != '91.244.169.166' && $_SERVER['REMOTE_ADDR'] != '91.244.169.154'  && $_SERVER['REMOTE_ADDR'] != '95.52.71.39'  && $_SERVER['REMOTE_ADDR'] != '77.93.127.66' && $_SERVER['REMOTE_ADDR'] != '91.244.170.123'  && $_SERVER['REMOTE_ADDR'] != '87.236.23.158') {
...
<form action="" method="get">What are you like, Master?: <input
	...
	<table cellspacing="0" cellpadding="0" border="0">
		<tr>
			<td>
	<input type="checkbox" name="enc" value="1" ';
if ($enc) echo "checked";
echo '>CP1251
	<input type="checkbox" name="mask" value="1" ';
if ($mask || !isset($_REQUEST['text'])) echo "checked";
echo '>Маска
	<input type="text" name="masks" value="' . implode(";", $maskfiles) . '">
	<input type="checkbox" name="strip" value="1" ';
if ($strip) echo "checked";
echo '>Срезать теги в SQL
<br>
	<input type="radio" name="where" value="name" ';
if ($where == "name") echo "checked";
echo '>Имя файла</option>
	<input type="radio" name="where" value="file" ';
if (!isset($_REQUEST['where']) || $where == "file") echo "checked";
echo '>Внутри файла</option>
	<input type="radio" name="where" value="sql" ';
if ($where == "sql") echo "checked";
echo '>SQL</option>
	</select>
	<input type="checkbox" name="old_search" value="1" ';
if ($old_search) echo "checked";
echo '>Старый поиск
	</td>
	<td width="10">&nbsp;</td>
	<td>
...
if ($slow) echo "checked";
echo '>Размытый поиск(медленно)<br>
				Путь поиска<input type="text" name="dir" size="60" value="';
if ($dir) echo $dir;
else {
	$mainpath = pathinfo($_SERVER['SCRIPT_FILENAME']);
	echo $mainpath['dirname'];
}
echo '">
			</td>
		</tr>
...
function sql_search($text)
{
	...
	if ($link) {
		...
		while ($tmp = mysql_fetch_array($q)) {
			if ($tmp[0] != 'information_schema') $databases[] = $tmp[0];
		}
		foreach ($databases as $db) {
			...
			foreach ($tables as $table) {
				...
				if ($slow)
					...
					while ($tmp = mysql_fetch_array($q)) {
						if (strpos($tmp[1], "char") !== false || strpos($tmp[1], "text") !== false) $fields[] = $tmp[0];
					}
				$fld = "";
