	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults.$Auth.$SendCrm) && $RIGHT >= "W" && check_bitrix_sessid())
	{
		...
		if($Auth)
		{
			if($_POST["DOMAIN"] && $_POST["TOKEN"])
			{
				...
				echo \Aspro\Functions\CAsproMaxCRM::query(
					$url,
					\Aspro\Functions\CAsproMaxCRM::$arCrmMethods['FLOWLU']['LIST_LEADS'],
					$arPostFields,
					array()
				);
			}
		...
		else
		{
			foreach($arTabs as $key => $arTab)
			{
				...
				if($domain && $token){
					...
					$result = json_decode(
						\Aspro\Functions\CAsproMaxCRM::query(
							$url,
							\Aspro\Functions\CAsproMaxCRM::$arCrmMethods['FLOWLU']['LIST_LEADS'],
							$arPostFields,
							array()
																	...
																	<table class="internal" style="width:100%;">
																		...
																		<tbody>
																			<?foreach($arFormResults as $arFormResult):?>
																				<?$arStatus = unserialize(Option::get($moduleID, 'CRM_SEND_FORM_'.$arFormResult["ID"], 'a:0:{}', $optionsSiteID));
