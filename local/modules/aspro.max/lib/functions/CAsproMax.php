...
if(!class_exists("CAsproMax"))
{
	class CAsproMax{
		...
		//log to file
		public static function set_log($type="log", $path="log_file", $arMess=[]){
		...
		public static function showRegionList(){
			...
			<?$frame->begin();?>
				...
				<?$template = strtolower((string) $arTheme["USE_REGIONALITY"]["DEPENDENT_PARAMS"]["REGIONALITY_VIEW"]["VALUE"]);
		...
		public static function checkActiveFilterPage($sefUrl = ''){
			...
			{
				global $APPLICATION;
				if(isset($sefUrl) && strripos((string) $sefUrl, "#SMART_FILTER_PATH#"))
				{
					...
					preg_match("/".$isSmartFilter."/i", (string) $APPLICATION->GetCurPage(), $arParseStr);
				}
			}
			return $arParseStr;
		}

		public static function getPricesID($arPricesID = [], $bUsePriceCode = false){
			$arPriceIDs = [];
			if($arPricesID)
			{
				global $USER;
				...
				if (!isset($priceTypeCache[$cacheKey]))
				{
					...
					$priceIterator = \Bitrix\Catalog\GroupAccessTable::getList(['select' => ['CATALOG_GROUP_ID'], 'filter' => ['@GROUP_ID' => $arUserGroups, 'CATALOG_GROUP_ID' => $arPricesID, 'ACCESS' => [\Bitrix\Catalog\GroupAccessTable::ACCESS_BUY, \Bitrix\Catalog\GroupAccessTable::ACCESS_VIEW]], 'order' => ['CATALOG_GROUP_ID' => 'ASC']]);
		...
		public static function showItemCounter($arAddToBasketData, $id, $arItemIDs, $arParams = [], $class = '', $svgSize = '', $bWrap = false, $bDetail = false){
			if(is_array($arParams) && $arParams):?>
				...
				foreach(GetModuleEvents(self::MODULE_ID, 'OnAsproShowItemCounter', true) as $arEvent) // event for manipulation item delay and compare buttons
					ExecuteModuleEventEx($arEvent, [$arAddToBasketData, $arItem, $arItemIDs, $arParams, &$html]);
		...
		public static function showShareBlock($class = ''){?>
			<?ob_start();?>
				...
				<div class="share hover-block <?=$class;?>">
					<div class="shares-block hover-block__item text-center colored_theme_hover_bg-block">
						...
						<?$APPLICATION->IncludeFile(SITE_DIR."include/share_buttons.php", [], ["MODE" => "html", "NAME" => GetMessage('CT_BCE_CATALOG_SOC_BUTTON')]);?>
					</div>
				</div>
			<?$html = ob_get_contents();
			ob_end_clean();
			...
			foreach(GetModuleEvents(self::MODULE_ID, 'OnAsproShowShareBlock', true) as $arEvent) // event for manipulation item delay and compare buttons
				ExecuteModuleEventEx($arEvent, [&$html, $class]);

			echo $html;?>
		<?}

		public static function getLinkedItems($arItem = [], $field = "SERVICES", $arParams = []){
			$arResult = [];
			$arSelect = ["ID", "IBLOCK_ID", "NAME", "DETAIL_PAGE_URL", "PROPERTY_LINK_GOODS_FILTER", "PROPERTY_LINK_GOODS"];
			...
			{
				$arItems = \CMaxCache::CIBLockElement_GetList(['CACHE' => ["TAG" => \CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_".$field."_ID"]), "GROUP" => "ID"]], ["IBLOCK_ID" => $arParams["IBLOCK_".$field."_ID"], "ACTIVE"=>"Y", "ACTIVE_DATE" => "Y"], false, false, $arSelect);
						...
						if($arItem2['~PROPERTY_LINK_GOODS_FILTER_VALUE'])
						{
							...
							if(
								...
								if($arTmpParams['CUSTOM_FILTER'])
								{
									$arFilter = [
             "LOGIC" => "AND",
             ["IBLOCK_ID" => $arParams["IBLOCK_ID"], "ACTIVE"=>"Y", "ID" => $arItem["ID"]],
             /*array(
             			"ID" => $arItem['~PROPERTY_LINK_GOODS_VALUE']
             		),*/
             $arTmpParams['CUSTOM_FILTER'],
         ];
									$arTmpItems = \CMaxCache::CIBLockElement_GetList(['CACHE' => ["TAG" => \CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"]), "GROUP" => "ID"]], $arFilter, false, false, ["ID"]);
		...
		public static function showItemOCB($arAddToBasketData, $arResult, $arParams = [], $bReturn = false, $class="btn-sm"){
			if(is_array($arParams) && $arParams):?>
				<?ob_start();?>
		...
		public static function prepareArray($arFields = [], $arReplace = [], $stamp = '_leads'){
			$arTmpFields = [];
			...
			if($formCode && $bCheckBlock):?>
				<div class="form-action">
					<div class="form-action__inner bordered rounded2 box-shadow animate-load" data-event="jqm" data-param-form_id="<?=$formCode?>" data-name="<?=$formCode?>">
						<?=\CMax::showIconSvg("icon colored", SITE_TEMPLATE_PATH.'/images/svg/'.strtolower((string) $formCode).'.svg', '', '', true, false);?>
		...
		{?>
			...
			foreach(GetModuleEvents(self::MODULE_ID, 'OnAsproShowSideFormLinkIcons', true) as $arEvent) // event
				ExecuteModuleEventEx($arEvent, [$bWrapper, &$html]);
			if(!$bReturn)
				echo $html;
			else
				return $html;?>
       ...
       'FEW_WEEKS' => Loc::getMessage('FEW_WEEKS_USE'),
       'FEW_MONTHS' => Loc::getMessage('FEW_MONTHS_USE'),
       'FEW_DAYS' => Loc::getMessage('FEW_DAYS_USE'),
       default => Loc::getMessage('FEW_YEAR_USE'),
   };
			return $timeFormat;
		}
		public static function encode($arItem = [], $options = null){
		...
		public static function showComments(){
			...
			if ($BLOG_DATA['COMMENT_ID'] && \Bitrix\Main\Loader::includeModule('blog')) {
				...
				$arFilter = ["BLOG_ID" => $BLOG_DATA['BLOG_DATA']['BLOG_ID'], "POST_ID" => $BLOG_DATA['COMMENT_ID']];
				...
				<?foreach($arPosts as $arPost):?>
					<div class="hidden" itemprop="review" itemscope itemtype="http://schema.org/Review">
						...
						<span style="display:none" itemprop="itemReviewed" itemscope itemtype="http://schema.org/Thing">
							<meta itemprop="name" content="<?=strip_tags((string) $arResult['NAME'])?>" />
						</span>
		...
		public static function showDiscountCounter($totalCount, &$arDiscount = [], $arQuantityData, $arResult, $strMeasure = '', $type = 'v2', $item_id = 0, $bWrapper = false){?>
			<?if(!$arDiscount && $item_id)
			{
				global $USER;
				$arUserGroups = $USER->GetUserGroupArray();?>
				<?$arDiscounts = \CCatalogDiscount::GetDiscountByProduct($item_id, $arUserGroups, "N", [], SITE_ID);?>
				<?$arDiscount=[];
				if($arDiscounts)
			...
			$bShowDiscont = ($arDiscount && $arDiscount["ACTIVE_TO"] && time() <= strtotime((string) $arDiscount["ACTIVE_TO"]));
			...
			if( $bShowDiscont || $bOffers):?>
							...
							<div class="quantity_block">
								<div class="values">
									<span class="item">
										...
										<span class="text"><?=($strMeasure ?: GetMessage("TITLE_QUANTITY"));?></span>
									</span>
								</div>
							</div>
						<?endif;?>
				...
				foreach(GetModuleEvents(self::MODULE_ID, 'OnAsproShowDiscountCounter', true) as $arEvent) // event for manipulation item img
					ExecuteModuleEventEx($arEvent, [$totalCount, $arDiscount, $arQuantityData, $arResult, $strMeasure, $type, &$html]);
		...
		public static function showResponsiblePreview($bAll = true){
			?>
				...
				foreach(GetModuleEvents(self::MODULE_ID, 'OnAsproShowResponsiblePreview', true) as $arEvent) // event for manipulation responsible preview
					ExecuteModuleEventEx($arEvent, [$bAll, &$html]);
		...
		public static function showCalculateDeliveryBlock($productId, $arParams, $bSkipPreview = false){
			...
			<?if($productId > 0 && $arParams['CALCULATE_DELIVERY'] !== 'NOT'):?>
				<?
				$bIndexBot = (isset($_SERVER['HTTP_ACCEPT_LANGUAGE']) && str_contains((string) $_SERVER['HTTP_USER_AGENT'], 'Lighthouse')); // is indexed yandex/google bot
				...
				foreach(GetModuleEvents(self::MODULE_ID, 'OnAsproShowCalculateDeliveryBlock', true) as $arEvent) // event for manipulation calculate delivery link
					ExecuteModuleEventEx($arEvent, [$productId, $arParams, &$html]);
		...
		public static function getPriceList($elementID, $arPricesID = [], $quantity = 1, $bUsePriceCode = false){
			$arPricesList = [];
			if($arPricesID)
			{
				...
				$arFilter = ['=PRODUCT_ID' => $elementID, '@CATALOG_GROUP_ID' => $arPricesID, ['LOGIC' => 'OR', '<=QUANTITY_FROM' => $quantity, '=QUANTITY_FROM' => null], ['LOGIC' => 'OR', '>=QUANTITY_TO' => $quantity, '=QUANTITY_TO' => null]];

				if(class_exists('\Bitrix\Catalog\PriceTable'))
				{
					$iterator = \Bitrix\Catalog\PriceTable::getList(['select' => $arSelect, 'filter' => $arFilter]);
				}
				else
				{
		...
		public static function sendResultToIBlock($WEB_FORM_ID, $RESULT_ID){
			...
			if(!$bAdminSection)
			{
				...
				if($arForm && $arForm['SID'] == 'REVIEW')
				{
					...
					\CFormResult::GetDataByID($RESULT_ID, [], $arResultFields, $arAnswers);

					if($arrAnswersVarname)
					{
						$el = new \CIBlockElement;

						$PROP = ['EMAIL' => $arrAnswersVarname[$RESULT_ID]['EMAIL'][0]['USER_TEXT'], 'POST' => $arrAnswersVarname[$RESULT_ID]['POST'][0]['USER_TEXT'], 'RATING' => $arrAnswersVarname[$RESULT_ID]['RATING'][0]['USER_TEXT']];

						$arLoadProductArray = ["IBLOCK_ID" => \CMaxCache::$arIBlocks[SITE_ID]["aspro_max_content"]["aspro_max_add_review"][0], "PROPERTY_VALUES"=> $PROP, "ACTIVE"=> "N", "NAME"=> $arrAnswersVarname[$RESULT_ID]['NAME'][0]['USER_TEXT'], "PREVIEW_TEXT"=> $arrAnswersVarname[$RESULT_ID]['REVIEW_TEXT'][0]['USER_TEXT'], "PREVIEW_PICTURE"=> \CFile::MakeFileArray($arrAnswersVarname[$RESULT_ID]['FILE'][0]['USER_FILE_ID'])];
			...
			if(
				...
				if($arAllMatchValues){
					...
					if($arPostFields){
						...
						if($crm === 'AMO_CRM'){
							$arOAuth = [];
							$arConfig = ['type' => 'AMO_CRM', 'siteId' => $SITE_ID];
							...
							$arHeaders = ['Authorization' => 'Bearer '.$arOAuth['accessToken']];
						}
						...
						foreach($arPostFields as $crm => $arFields){
							...
							if($crm == 'AMO_CRM'){
								...
								// prepare array
								$arFieldsLeadTmp = self::prepareArray($arFields, ['name', 'tags', 'budget'], '_leads');
								if($arCustomFields && $arCustomFields['leads']){
									foreach($arCustomFields['leads'] as $key => $arProp){
										if($arFields[$key.'_leads']){
											$arFieldsLeadTmp['custom_fields'][] = ['id' => $key, 'values' => [['value' => $arFields[$key.'_leads']]]];
										}
										elseif(isset($arProp['ENUMS']) && $arProp['ENUMS']){
											foreach($arProp['ENUMS'] as $key2 => $value){
												if($arFields[$key.'_'.$key2.'_leads']){
													$arFieldsLeadTmp['custom_fields'][] = ['id' => $key, 'values' => [['value' => $arFields[$key.'_'.$key2.'_leads'], 'enum' => $value]]];
												}
											}
										}
									}
								...
								$arFieldsLead = ['request' => ['leads' => ['add' => [$arFieldsLeadTmp]]]];
							}
							...
							if(isset($arCrmResult['response'])){
								if($crm == 'AMO_CRM' && $arCrmResult['response']['leads']){
									...
									if($arFields['notes_leads']){
										$arFieldsNote = ['request' => ['notes' => ['add' => [['element_id' => $leadID, 'element_type' => 2, 'note_type' => 4, 'text' => $arFields['notes_leads']]]]]];
									...
									if($arCustomFields && $arCustomFields['companies']){
										// prepare array
										$arFieldsCompanyTmp = self::prepareArray($arFields, ['name', 'tags'], '_companies');
										$arFieldsCompanyTmp['linked_leads_id'] = [$leadID];

										foreach($arCustomFields['companies'] as $key => $arProp){
											if($arFields[$key.'_companies']){
												$arFieldsCompanyTmp['custom_fields'][] = ['id' => $key, 'values' => [['value' => $arFields[$key.'_companies']]]];
											}
											elseif(isset($arProp['ENUMS']) && $arProp['ENUMS']){
												foreach($arProp['ENUMS'] as $key2 => $value){
													if($arFields[$key.'_'.$key2.'_companies']){
														$arFieldsCompanyTmp['custom_fields'][] = ['id' => $key, 'values' => [['value' => $arFields[$key.'_'.$key2.'_companies'], 'enum' => $value]]];
													}
												}
											}
										}
										$arFieldsCompany = ['request' => ['contacts' => ['add' => [$arFieldsCompanyTmp]]]];
									...
									//add contact
									$arFieldsContactTmp = self::prepareArray($arFields, ['name', 'tags'], '_contacts');
									$arFieldsContactTmp['linked_leads_id'] = [$leadID];
									...
									if($arCustomFields && $arCustomFields['contacts']){
										foreach($arCustomFields['contacts'] as $key => $arProp){
											if($arFields[$key.'_contacts']){
												$arFieldsContactTmp['custom_fields'][] = ['id' => $key, 'values' => [['value' => $arFields[$key.'_contacts']]]];
											}
											elseif(isset($arProp['ENUMS']) && $arProp['ENUMS']){
												foreach($arProp['ENUMS'] as $key2 => $value){
													if($arFields[$key.'_'.$key2.'_contacts']){
														$arFieldsContactTmp['custom_fields'][] = ['id' => $key, 'values' => [['value' => $arFields[$key.'_'.$key2.'_contacts'], 'enum' => $value]]];
													}
												}
											}
										}
									...
									$arFieldsContact = ['request' => ['contacts' => ['add' => [$arFieldsContactTmp]]]];
								...
								if(
									...
									$arFormResultOption = unserialize(Option::get(self::MODULE_ID, 'CRM_SEND_FORM_'.$RESULT_ID, 'a:0:{}', $SITE_ID));
		...
		public static function checkAvailable($arFilter, $options = [], $arParams = []) {
			global $arRegion;
			...
			$resElements = \CMaxCache::CIblockElement_GetList( ["CACHE" => ["TAG" => \CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])]] , $arFilter, []);

			return $resElements;
		}

		public static function getCustomFunc($method = '') {
			$className = end(explode('\\', self::class));
			$methodCall = $method.$className;
			$classCall = self::class.'Custom';
		...
		public static function showBonusComponentDetail($arResult) {
			...
			if(\CMax::GetFrontParametrValue("BONUS_SYSTEM") === 'LOGICTIM' && \CModule::IncludeModule('logictim.balls')){
				$APPLICATION->IncludeComponent(
					...
					"aspro_max",
					["COMPONENT_TEMPLATE" => ".default", "COMPOSITE_FRAME_MODE" => "A", "COMPOSITE_FRAME_TYPE" => "AUTO", "ITEMS" => ["ITEMS"=>$arResult]]
				);
			}
		}
		...
		public static function showBonusComponentList($arResult) {
			...
			if(\CMax::GetFrontParametrValue("BONUS_SYSTEM") === 'LOGICTIM' && \CModule::IncludeModule('logictim.balls')){
				$APPLICATION->IncludeComponent(
					"logictim:bonus.catalog",
					"aspro_max",
					["COMPONENT_TEMPLATE" => ".default", "COMPOSITE_FRAME_MODE" => "A", "COMPOSITE_FRAME_TYPE" => "AUTO", "ITEMS" => $arResult["ITEMS"]]
				);
			}
		}
		...
		public static function showBottomPanel() {
			...
			if ($arTheme['BOTTOM_ICONS_PANEL']["VALUE"] == 'Y') {
				...
				if ($iblockID) {
					...
					$arItems = \CMaxCache::CIBLockElement_GetList(['SORT' => 'ASC', 'ID' => 'ASC', 'CACHE' => ['TAG' => \CMaxCache::GetIBlockCacheTag($iblockID)]], $arFilter, false, false, ['ID', 'NAME', 'IBLOCK_ID', 'PROPERTY_IMG', 'PROPERTY_TYPE', 'PROPERTY_URL', 'PROPERTY_SHOW_TEXT']);?>
              ...
              if ($arBasketPrices) {
               ...
               $count = count($compare_items);
									...
									<a href="<?=$url?>" class="bottom-icons-panel__content-link<?=($bActive ? ' bottom-icons-panel__content-link--active' : ' dark-color');?> bottom-icons-panel-item_<?=$arProperty['XML_ID']?>" title="<?=$title;?>">
										...
										<?if ($arItem['PROPERTY_IMG_VALUE']) {?>
											<span class="bottom-icons-panel__content-picture-wrapper bottom-icons-panel__content-link--display--block<?=($bShowText ? ' bottom-icons-panel__content-picture-wrapper--mb-3' : '');?>">
												<?$arImg = \CFile::ResizeImageGet($arItem['PROPERTY_IMG_VALUE'], ['width' => 60, 'height' => 60], BX_RESIZE_IMAGE_PROPORTIONAL_ALT);
												if(is_array($arImg)) {
													if(str_contains((string) $arImg["src"], ".svg")) {?>
					...
					foreach(GetModuleEvents(self::MODULE_ID, 'OnAsproShowBottomPanel', true) as $arEvent) // event for manipulation item
						ExecuteModuleEventEx($arEvent, [$iblockID, $arItems, &$html]);
		...
		public static function shemaOrganization(){
			...
			<?if($arRegion):
				$adress = $arRegion['PROPERTY_ADDRESS_VALUE']['TEXT'];
