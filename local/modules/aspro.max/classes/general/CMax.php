...
class CMax{
	...
	public static function showContactDesc(){
		...
		//$bAddr = ($arRegion ? $arRegion['PROPERTY_ADDRESS_VALUE']['TEXT'] : self::checkContentFile(SITE_DIR.'include/contacts-site-address.php'));
		$bDesc = ($arRegion && is_array($arRegion['PROPERTY_REGION_TAG_SHEDULLE_VALUE']) ? $arRegion['PROPERTY_REGION_TAG_CONTACT_TEXT_VALUE']['TEXT'] : self::checkContentFile(SITE_DIR.'include/contacts-about.php'));
		$bRegionContact = (\Bitrix\Main\Config\Option::get(self::moduleID, 'SHOW_REGION_CONTACT', 'N') == 'Y');
	...
	public static function showContactSchedule($txt = '', $wrapTable = true, $class = '', $icon = 'WorkingHours_lg.svg', $subclass = ''){
		...
		if ($USER->IsAdmin()) {
			/* echo '<pre>';
			var_dump($arRegion['PROPERTY_REGION_TAG_SHEDULLE_VALUE']);
			echo '</pre>'; */
		};

		$bAddr = ($arRegion && $bRegionContact && is_array($arRegion['PROPERTY_REGION_TAG_SHEDULLE_VALUE'])  ? $arRegion['PROPERTY_REGION_TAG_SHEDULLE_VALUE']['TEXT'] : self::checkContentFile(SITE_DIR.'include/contacts-site-schedule.php'));
		?>
		<?if($arRegion):?>
			<?$frame = new \Bitrix\Main\Page\FrameHelper('header-allcaddr-block'.$iCalledID);?>
			<?$frame->begin();?>
		...
		<?if($bAddr):?>
					<div class="property schedule">
						<div class="title font_upper muted"><?=$txt;?></div>
						<?if($arRegion && is_array($arRegion['PROPERTY_REGION_TAG_SHEDULLE_VALUE']) && $bRegionContact):?>
	...
	public static function GetAddToBasketArray(&$arItem, $totalCount = 0, $defaultCount = 1, $basketUrl = '', $bDetail = false, $arItemIDs = array(), $class_btn = "small", $arParams=array()){
		...
		if($arItem["OFFERS"]){
			...
			//var_dump($arItem ["PROPERTIES"]["PRICE"]);
			if($bPriceExists = (isset($arItem["MIN_PRICE"]) && $arItem["MIN_PRICE"]["VALUE"] > 0) || (isset($arItem["PROPERTIES"]["PRICE"]) && $arItem["PROPERTIES"]["PRICE"]["VALUE"] > 0)){
				// price exists
