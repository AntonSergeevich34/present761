...
class FrontController
{
    ...
    public function run()
    {
        ...
        $filterRuleIdParam = $this->request->getQuery('filter_rule_id');
    
        error_log("Route: " . $routeParam);
        error_log("ID: " . $idParam);
        error_log("Filter Rule ID: " . $filterRuleIdParam);
        $config = [
    ...
    private function getMethodArgs($class, $method)
{
    ...
    foreach ($methodReflection->getParameters() as $param) {
        $paramName = mb_strtolower($param->getName());
        if ($_value = $this->request->getQuery($param->getName())) {
            $result[$paramName] = $_value;
        } else {
