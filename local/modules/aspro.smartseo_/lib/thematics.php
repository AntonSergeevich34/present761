...
class Thematics {
	...
	public static function clear (array $arParams = []) :bool {
		...
		$defaultZipPath = $_SERVER['DOCUMENT_ROOT'].'/local/modules/'.self::ASPRO_MODULE_NAME.'/tmp_zip_files';
	...
	public static function copy (array $arParams = []) :bool {
		...
		$defaultZipPath = $_SERVER['DOCUMENT_ROOT'].'/local/modules/'.self::ASPRO_MODULE_NAME.'/tmp_zip_files/files/'.$moduleID;
		$zipDir = is_string($zipDir) && strlen($zipDir) ? $zipDir : $defaultZipPath;
		...
		foreach ($_SESSION[$templateID]['FILES'] as $unzipIndex => $arFile) {
			if (!in_array($arFile['NAME'], $_SESSION[$templateID]['UNZIPED_FILES'])) {
				...
				if ($bUpdate) {
					...
					$moduleDir = $_SERVER['DOCUMENT_ROOT'].'/local/modules/'.$moduleID;
