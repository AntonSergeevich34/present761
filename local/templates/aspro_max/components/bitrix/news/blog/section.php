...
<?else:?>
	...
	$numPage = $context->getRequest()->get("PAGEN_2") ?? 0;
	...
	$arElement = CMaxCache::CIblockElement_GetList(array("CACHE" => array("TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"]), "MULTI" => "Y"), $arParams['SORT_BY1'] => $arParams['SORT_ORDER1']), $arItemFilter, false, $arGroup, $arSelect);
	...
	foreach($arElement as $key=>$element):
		...
		if($element['PREVIEW_PICTURE']){
			$arSchema[$key]['image'][] = $_SERVER['SERVER_NAME'].CFile::GetPath($element['PREVIEW_PICTURE']);
		}
		if($element['DETAIL_PICTURE']){
			$arSchema[$key]['image'][] = $_SERVER['SERVER_NAME'].CFile::GetPath($element['DETAIL_PICTURE']);
		}
	endforeach;
	?>
