...
?>
    <script>
        ...
        BX.message({
            ...
            // Copying fields with the same name
            ACRIT_EXP_COPY_SIMILAR_FIELD: '<?=Helper::getMessage('ACRIT_EXP_COPY_SIMILAR_FIELD');?>'
        });
    </script><?
...
if ($intProfileID > 0) {
    ...
    $strPageTitle = Loc::getMessage('ACRIT_EXP_PAGE_TITLE_EDIT', ['#ID#' => $intProfileID]);
}
...
$bCopy = $arGet['copy'] == 'Y';
if ($bCopy) {
    $strPageTitle = Loc::getMessage('ACRIT_EXP_PAGE_TITLE_COPY', ['#ID#' => $intProfileID]);
}
    ...
    if (strlen($arProfile['FORMAT'])/* && is_array($arPlugins[$arProfile['PLUGIN']])*/) {
        ...
        if (is_array($arProfilePlugin)) {
            ...
            print Helper::showError(Loc::getMessage('ACRIT_EXP_ERROR_FORMAT_NOT_FOUND_TITLE'),
                Loc::getMessage('ACRIT_EXP_ERROR_FORMAT_NOT_FOUND_DETAILS', [
...
$arProfile['ACTIVE'] = in_array($arProfile['ACTIVE'], ['Y', 'N']) ? $arProfile['ACTIVE'] : 'Y';
    ...
    switch ($strAjaxAction) {
        ...
        case 'export_execute':
            ...
            if (Helper::call($strModuleId, 'Profile', 'isLocked', [$intProfileID])) {
                ...
                $arJsonResult['LockedHtml'] .= Helper::showNote(Loc::getMessage('ACRIT_EXP_LOCK_NOTICE', [
        ...
        case 'cron_setup':
            ...
            if ($strRight == 'W') {
                ...
                if ($bAdd) {
                    ...
                    if ($bResult) {
                        Log::getInstance($strModuleId)->add(Loc::getMessage('ACRIT_EXP_AJAX_CRON_SETUP_SUCCESS', [
                            '#COMMAND#' => $arCli['COMMAND'],
                        ]), $intProfileID);
                    } else {
                        Log::getInstance($strModuleId)->add(Loc::getMessage('ACRIT_EXP_AJAX_CRON_SETUP_ERROR', [
                            '#COMMAND#' => $arCli['COMMAND'],
                        ]), $intProfileID);
                    }
                } else {
        ...
        case 'profile_unlock':
            ...
            if ($obResult->isSuccess()) {
                $arJsonResult['Success'] = true;
                $arSession = unserialize($arProfile['SESSION'], ['allowed_classes' => false]);
                $arSession = is_array($arSession) ? $arSession : [];
                if ($arSession['HISTORY_ID'] > 0) {
                    #$obResult = History::update($arSession['HISTORY_ID'], array('STOPPED' => 'Y'));
                    $obResult = Helper::call($strModuleId, 'History', 'update', [$arSession['HISTORY_ID'], ['STOPPED' => 'Y']]);
                    $arJsonResult['Success'] = $obResult->isSuccess();
                }
            }
            break;
        ...
        case 'categories_update':
            ...
            if ($bContinue) {
                ...
                $arJsonResult['Message'] = Helper::showNote(Helper::getMessage('ACRIT_EXP_UPDATE_CATEGORIES_UNAVAILABLE', [
        ...
        case 'check_lock':
            ...
            if (Helper::call($strModuleId, 'Profile', 'isLocked', [$arProfile])) {
                ...
                print Helper::showNote(Loc::getMessage('ACRIT_EXP_LOCK_NOTICE', [
        ...
        case 'console_execute':
            ...
            if ($USER->isAdmin()) {
                ...
                $arModules = ['iblock', 'catalog', 'sale', 'currency'];
                ...
                $strContent .= Loc::getMessage('ACRIT_EXP_AJAX_CONSOLE_TIME', [
                ...
                if ($arProfile['ACTIVE'] == 'Y') {
                    ...
                    if (!Helper::call($strModuleId, 'Profile', 'isLocked', [$arProfile])) {
                        ...
                        if ($arJsonResult['Pid'] > 0) {
                            $arJsonResult['Success'] = true;
                            $arJsonResult['SuccessMessage'] = Loc::getMessage('ACRIT_EXP_RUN_BACKGROUND_SUCCESS', [
        ...
        case 'add_multicondition_item':
            ...
            $strSuffix = call_user_func([$arTypes['MULTICONDITION']['CLASS'], 'getRandomSuffix']);
            ...
            $arJsonResult['HTML'] = $obValue->showAddCondition($strSuffix, []);
            unset($obValue);
            break;
        //
...
foreach (\Bitrix\Main\EventManager::getInstance()->findEventHandlers($strModuleId, 'OnGetAdditionalTabs') as $arHandler) {
    ExecuteModuleEventEx($arHandler, [&$arTabs]);
}
