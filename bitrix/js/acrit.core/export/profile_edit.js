...
function acritExpDetectSafari(){
	if(window.acritExpSafari == undefined){
		window.acritExpSafari = /safari/.test(navigator.userAgent.toLowerCase()) 
			&& !(/chrome/.test(navigator.userAgent.toLowerCase()));
	}
}

// Wrapper for jQuery.clone, that correctly copies selected value in <select>
if($.fn && !$.fn.acritClone){
	(function (original) {
		jQuery.fn.acritClone = function () {
...
function acritExpPluginLoadSettings(saveValues) {
	...
	acritExpAjax('load_plugin_settings', currentValues, function(JsonResult, textStatus, jqXHR){
		...
		acritExpAfterPluginLoadSettings(JsonResult);
		acritExpFixBottomPanelVisible();
		acritExpHandleAjaxError(jqXHR, false);
	}, function(jqXHR){
		acritExpHandleAjaxError(jqXHR, true);
	}, true);
}
function acritExpAfterPluginLoadSettings(JsonResult) {
	// Actual, for example, for Avito
	$('#checkbox_STEP_BY_STEP').trigger('change');
}
function acritExpFixBottomPanelVisible() {
...
$(document).ready(function(){
	// Select2
	window.acritExpSelect2Matcher = function(params, item){
		function optionIsMatch(option, search){
			return [option.text, option.value, option.title].join(' ').toUpperCase().indexOf(searchText) != -1;
		}
...
$(document).ready(function(){
	...
	AcritExpPopupSelectField.FilterFields = function(){
		...
		if(searchText=='') {
			$('optgroup', fieldList).acritExpToggleListItem(true);
			$('optgroup', fieldList).acritExpToggleListItem(false);
			$('optgroup', fieldList).acritExpToggleListItem(true);
			$('optgroup option', fieldList).acritExpToggleListItem(true);
			fieldList.children('option').acritExpToggleListItem(false);
		}
		else {
			var found = false;
			fieldList.children('option').acritExpToggleListItem(true);
			$('optgroup', fieldList).acritExpToggleListItem(false);
			$('optgroup option', fieldList).acritExpToggleListItem(false).each(function(){
				var search = $(this).val().toLowerCase() + ' ' + $.trim($(this).text()).toLowerCase();
				var matched = search.indexOf(searchText) > -1;
				if(matched){
					$(this).acritExpToggleListItem(true).closest('optgroup').acritExpToggleListItem(true);
					found = true;
				}
			});
			if(found) {
...
$(document).delegate('a[data-role="sections_select2_mode"]', 'click', function(e){
	e.preventDefault();
	let select = $('select[data-role="categories-list"]');
	console.log($('select[data-role="categories-list"]'));
	if(select.is('[data-select2-id]')){
		select.select2('destroy');
	}
	else{
		select.select2().next().css('width', '100%');
	}
});
$(document).ready(function(){
...
/* Copy field values (Wildberries, Ozon) */
$(document).delegate('td.acrit-exp-fields-table-field', 'click', function(e){
	// Check Ctrl is pressed
	if(e.ctrlKey){
		let
			row = $(this).closest('tr[data-field]'),
			code = row.attr('data-field'),
			name = row.attr('data-name'),
			table = $(this).closest('table.acrit-exp-fields-table'),
			field = row.attr('data-name'),
			fields = $('tr[data-role="field_row"][data-name="'+field+'"]', table).not(row),
			message = BX.message('ACRIT_EXP_COPY_SIMILAR_FIELD').replace(/#CODE#/g, code).replace(/#NAME#/g, name);
		// Confirm copying (if no similar fields, do nothing)
		if(fields.length && confirm(message)){
			// Search same fields
			$('tr[data-role="field_row"][data-name="'+field+'"]', table).not(row).each(function(){
				var
					newCode = $(this).attr('data-field'),
					regExp = new RegExp(code, "g"),
					tdType = $('.acrit-exp-fields-table-type', row).acritClone(),
					tdValue = $('.acrit-exp-fields-table-value', row).acritClone(),
					tdParams = $('.acrit-exp-fields-table-settings', row).acritClone();
				// Put cloned cells to new place
				$('.acrit-exp-fields-table-type', this).replaceWith(tdType);
				$('.acrit-exp-fields-table-value', this).replaceWith(tdValue);
				$('.acrit-exp-fields-table-settings', this).replaceWith(tdParams);
				// Change inputs name (replace old code to new code)
				$('input,select,textarea', this).filter('[name]').each(function(){
					$(this).attr('name', $(this).attr('name').replace(regExp, newCode));
				});
			});
		}
	}
